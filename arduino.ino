...
void loop() {
 String computerText = Serial.readStringUntil('@');
  ...
  String command = getValue(computerText, ' ',0);
    ...
    if (command == "right" || command == "يمين" || command == "رايت" || command == "Right") {
      base.write(base_angle -= 20);
    }
    if (command == "left" || command == "شمال" || command == "Left" || command == "لفت") {
     base.write(base_angle += 20);
    }

    if (command == "top" || command == "فوق" || command == "توب" || command == "Top") {
      shoulder.write(shoulder_angle -= 20);
    }

   if (command == "bottom" || command == "تحت" || command == "بوتوم" || command == "Bottom") {
     shoulder.write(shoulder_angle += 20);
    }
